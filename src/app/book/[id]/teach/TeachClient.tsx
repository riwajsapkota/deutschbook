...
export default function TeachClient({ chapterId, chapterTitle, theory, exerciseCount }: Props) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      <p className="text-sm text-slate-600 mb-5">Step-by-step teaching mode</p>
      ...
      {isDone ? (
        <div className="bg-white border border-green-200 rounded-lg px-6 py-10 text-center">
          ...
          <p className="text-sm text-slate-600 mb-6">Ready to put it into practice?</p>
        ...
        <div className="bg-white border border-slate-200 rounded-lg px-6 py-6 mb-6">
          ...
          <ReactMarkdown
            ...
            components={{
              ...
              p: ({ children }) => <p className="text-sm text-slate-900 leading-relaxed mb-3">{children}</p>,
              ul: ({ children }) => <ul className="list-disc ml-5 mb-3 space-y-1 text-sm text-slate-900">{children}</ul>,
              ol: ({ children }) => <ol className="list-decimal ml-5 mb-3 space-y-1 text-sm text-slate-900">{children}</ol>,
              li: ({ children }) => <li className="leading-relaxed">{children}</li>,
              strong: ({ children }) => <strong className="font-semibold">{children}</strong>,
              em: ({ children }) => <em className="italic text-slate-600">{children}</em>,
              code: ({ children }) => <code className="bg-slate-100 text-blue-800 px-1.5 py-0.5 rounded text-xs font-mono">{children}</code>,
              table: ({ children }) => (
                <div className="overflow-x-auto mb-4">
                  <table className="w-full text-sm border-collapse border border-slate-200">{children}</table>
                </div>
              ),
              th: ({ children }) => <th className="bg-slate-50 border border-slate-200 px-3 py-2 text-left font-medium text-blue-900">{children}</th>,
              td: ({ children }) => <td className="border border-slate-200 px-3 py-2 text-slate-900">{children}</td>,
              blockquote: ({ children }) => <blockquote className="border-l-4 border-blue-300 pl-4 italic text-slate-600 mb-3">{children}</blockquote>,
            }}
          >
            {current.content}
          </ReactMarkdown>
        ...
        <div className="flex gap-3 mb-8">
          {step > 0 && (
            <button
              ...
              className="bg-white border border-slate-300 text-slate-700 px-5 py-2 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors"
            >
              Back
            </button>
          )}
      ...
      <div className="border-t border-slate-200 pt-6">
        <h3 className="text-sm font-semibold text-slate-700 mb-3">Ask a question</h3>
          ...
          <div className="space-y-3 mb-4 max-h-64 overflow-y-auto">
            ...
            {asking && (
              <div className="flex justify-start">
                <div className="bg-slate-100 border border-slate-200 rounded-lg px-4 py-2 text-sm text-slate-700 italic">
                  Thinking...
                </div>
              </div>
            )}
        ...
        <div className="flex gap-2">
          <input
            ...
            disabled={asking}
            className="flex-1 border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
          />
          <button
            onClick={handleAsk}
